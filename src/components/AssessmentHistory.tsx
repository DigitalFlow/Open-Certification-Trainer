...
export default class AssessmentHistory extends React.PureComponent<IBaseProps, AssessmentHistoryState> {
  ...
  render() {
    ...
    const questionRatio = calculateScorePerQuestion(this.state.previousSessions);

    let sortable = new Array<Array<string | number>>();

    for (const questionId in questionRatio) {
      const item = [questionId, questionRatio[questionId]];
      sortable.push(item);
    }
    ...
    sortable = sortable.map(arr => {
      const question = this.state.certification.questions.find(q => q.id === arr[0]);
