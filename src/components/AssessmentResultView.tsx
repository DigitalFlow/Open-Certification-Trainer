...
export default class AssessmentResultView extends React.PureComponent<AssessmentResultViewProps, AssessmentResultViewState> {
  ...
  render() {
    ...
    const correctAnswerCount = questions.reduce((acc: number, val: Question) => {
      const answeredCorrectly = checkIfAnsweredCorrectly(val.answers, this.props.session.answers[val.id].reduce((acc: IAssociativeArray<boolean>, val: string) => {
        acc[val] = true;
        return acc;
      }, { }));
